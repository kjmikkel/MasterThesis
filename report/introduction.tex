...
While important in a real world application, I will in this thesis not concern myself with security, neither in the form of the authentication, anonymity of the sender or [something about rooting I think it might be possible]. For a treatment of this subject see \cite{trustedRouting}. Furthermore, if there does not exist a path between two nodes, I will not make any guarantee that the message will ever arrive at the sink. \todo{finish this}
...
\defi{The Routing Graph is the graph that a message has to navigate through in order to go from the source to the sink. In most cases this graph is not unique.}
\defi{Geographic movement is when a node changes its position, this may or may not affect the topology of the routing graph}
\defi{Topological movement is when the movement in the node causes a change in the routing}
\defi{When a node $n_x$ can transmit information to another node $n_y$ (i.e. $n_y$ lies within the range of $n_x$ transmitter), then I will say that $n_x$ can communicate with $n_y$}
\defi{When two nodes $n_x$ and $n_y$ can communicate with each other, I will say that they are adjacent in the graph}
\defi{When a node $n_x$ can transmit information to another node $n_y$ (i.e. $n_y$ lies within the range of $n_x$ transmitter), then I will say that $n_x$ is adjacent to $n_y$}
\defi{Given two nodes $n_x$ and $n_y$, I will say that there exists a bidirectional link between them, when $n_x$ is adjacent to $n_y$ and $n_y$ is adjacent to $n_x$.}
\defi{When 2 nodes have a unidirectional link, and they are connected in the underlying routing graph, I will say that the two nodes are neighbours. I find this clarification necessary since }
