...
In the case of \manet where there are some hardware in which some nodes are stationary, it might be preferable to use these stationary nodes as a subset of the \ch, since it would cut down on the amount of administration 

A routing algorithm for the 
\begin{description}
...
\begin{description}
\item[The NS2 network simulator]
\item[GloMoSim]
\item[Opnet]
\end{description}
