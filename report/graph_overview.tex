...
If the algorithm chooses the first way, then there has been established two fundamnetally differenet principles that can be applied.
\begin{description}
\item[Destination Sequential Distance Vector (DSDV):] With the DSDV, every node tries to remember as many as possible paths to the other nodes in the graph. The mapping from nodes to paths can either just be the next neighbor that a message needs to be routed to, or the entire path. Some solutions employing DSDV tries to create the entire DSDV at the start and then maintian it for the lifetime of the network, while others build it piece-mail, adding information as it becomes relevant. The advantage of DSDV is that when the information is correct it saves a lot of work, and is fast, while the disadvantage is that it is memory intensive and thus has problems scaling to large \manet, and will be prone to contain outdated information. \cite{AODV}.

\item[Ad-Hoc On-demand Distance Vector (AODV):] With the AODV   \cite{larMotion, AODV}.
\end{description}

Routing algorithms suited for \manet s can be sorted into the following categories:
\begin{description}
\item[Flooding:] The simplest of the routing algorithms. The entire network is flooded with the message, ensuring that the message eventually reaches the sink. We can be sure that the message will not flood the network forever, thanks to the recording system detailed in section \ref{record-recived}. If the sink can be reached from the source, then flooding is  guarantied to work, but uses a lot of bandwidth \cite{larMANET, Sur2}. 

\item[Restricted Direction Routing (RDR):] Restricted Directed Routing is an algorithm that can be described as a kind of directed flooding. Let G be a geometrical shape. G is now placed such that it now contains both the source and the sink are both contined in G (preferably with a non-zero margian, to counter act any movement on the sinks part). The source now floods all nodes inside G, hopefully reaching the sink. Once a route to the sink has been established, the informat

The there is no standard shape, but possible choices includes Linear Swept Sphere, Circle or Rectangle \todo{Include figure of them all}. Obviously, RDR is only applicaple when the source has an idea of the location of the sink, otherwise another algorithm (like flooding) has to be employed. In the case where both nodes wants to contact each other (say, in situations where timers are involved), a bi-directional RDR can take place, where each node tries to find each other, until both diffusion reach each other, and a path can be established.
