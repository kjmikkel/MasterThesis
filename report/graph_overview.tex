...
\tikfig{different_radius}{A node with different broadcast ranges. The first solid circle is the default broadcast range of node $a$, which does not reach node $b$. The second circle is the maximum range that node $a$ can broadcast, which does reach node $b$.}
