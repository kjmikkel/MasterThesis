...
\tigfig{hidden_terminal}{The hidden terminal problem. In this example node c is communicating with node b (illustrated by the grey colour in node b's transmitting area) Since the transmission does not reach node a, it is not possible for node a to perform collision avoidance.}
...
\tigfig{node_different_radius}{Two nodes with different broadcast ranges. Node a is not adjacent to node b, but node b is adjacent to node a -- thus only one-way communication could take place.}

Under optimal conditions a node has a uniform circular broadcast range. If node a and node b are in each other broadcast radii, then they can communicate with each other. Thus it becomes clear that for any \manet we are going to deal with a communication network, it would seem logical to represent the connections between the nodes as a graph, where the nodes of the graphs are the \manet nodes, and the edges of the graph indicates that both nodes are within each others broadcast radii. The problem of sending a message from the source to the sink then becomes a graph routing problem (but not necessary a shortest-path routing problem, since factors like fairness or maximization of the lifetime of the \manet becomes issues in practice). 

The next step is then to identify which kind graphs would be suitable for \manet. I have not been able to find a consensus on the nature of the optimal graph.


 nodes will be able to broadcast and receive signals to/from nodes in a given radius around them, the most routing graph with the maximum number of edges will not be planar. Furthermore, while most routing algorithms assume so, the edges do not have to be bidirectional. One could easily imagine a scenario where heterogeneous ad-hoc nodes had different broadcast radii, leading to cases where certain nodes could receive other nodes broadcast through 1 jump, but would have to route through several nodes to get the message back.
