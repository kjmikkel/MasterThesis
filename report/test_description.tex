...
\eqa{
F &=& \sqrt{j^2 - n^2}\\
\frac{d(s,t)}{2} &=& \sqrt{(2 * d(s,t))^2 - n^2}\\
(\frac{d(s,t)}{2})^2 &=& 4 * d(s,t)^2 - n^2\\
n^2 &=& 4 * d(s,t)^2 - (\frac{d(s,t)}{2})^2\\
n &=& \sqrt{4 * d(s,t)^2 - (\frac{d(s,t)}{2})^2}\\
}
...
\item If at any point $p > \ro q$, where $\ro$ is a constant defined before routing starts, then route the message to the node closest to the sink and begin greedy routing again.
\end{enumerate}

Both GOAFR and GOAFR+ has proved to be theoretical asymptotically worst-case optimal and average-case efficient. Due to space demands I will not try to prove these claims here, but will instead refer the interested reader to \cite{gopher} and \cite{gopher+}. \todo{make sure I understand what it means and I am not misusing it} 
