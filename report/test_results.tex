...
This section shows the results I have found for the test of the 4 different routing algorithms. The raw results can be found in section~\ref{routing_raw_results}.

\subsubsection{Percentage of success}
\FloatBarrier
\graphResult{graph/percentage_graph_500}{This graph shows the percentage of success in delivering messages for the various routing algorithms for the the test on the 500 by 500 area.}{percent_500}
\graphResult{graph/percentage_graph_750}{This graph shows the percentage of success in delivering messages for the various routing algorithms for the the test on the 750 by 750 area.}{percent_750}
\FloatBarrier

\subsubsection{Number of hops}
\FloatBarrier
\graphResult{graph/hop_graph_500}{This graph shows the average number of hops for the various routing algorithms. Please take note of the dip for the DSDV at 60 nodes. This graph is for the tests on the 500 by 500 area.}{hop_500}
\graphResult{graph/hop_graph_750}{This graph shows the average number of hops for the various routing algorithms. Please take note of the dip for the DSDV at 90 nodes. This graph is for the tests on the 750 by 750 area.}{hop_750}
\FloatBarrier

\subsubsection{Amount of time}
\FloatBarrier
\graphResult{graph/time_graph_500}{This graph shows the amount of time used for the various routing algorithms to sent successful messages. This graph is for the tests on the 500 by 500 area.}{time_500}
\graphResult{graph/time_graph_750}{This graph shows the amount of time used for the various routing algorithms to sent successful messages. This graph is for the tests on the 750 by 750 area.}{time_750}
\FloatBarrier
...
\graphResult{spanner/dist_percent}{This graph show the ratio between the values from the \ac{gabe} and the \ac{rng} when compared to the values of the \ac{ucg}. The ratio's for \ac{ucg} has been omitted, since it by definition will be 1. The solid lines indicates the ratio of the total Euclidean distance, while the dashed lines indicates the ratio of total hops. The values used in the creation of this graph can be found in the spanner tables in the appendix under ``Ratio of values compared to the UCG''}{dist_percent}
