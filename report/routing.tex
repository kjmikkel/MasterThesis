...
As mentioned in Section~\ref{section:fundamental_graph_routing}, the problem of sending a message from the source to the sink  becomes a graph routing problem \footnote{But not necessary a shortest-path routing problem, since factors like fairness or maximisation of the lifetime of the \manet becomes issues in practise}.
...
\tikfig{rdr}{The grey background area is the area in which routing take place (any message that moves outside will be discarded). The black lines between the nodes represent a bidirectional link, while the grey background between nodes represent the valid bidrectional links inside the grey area. It is clear that the shape of the flooding will have a large effect on the number of nodes that will receive the transmission.}
...
\section{Problems introduced with node movement}
...
%\tikfig{changed_route}{In figure \ref{fig:changed_route1} we see that the routing algorithm has found a route $r$ (the path with the grey background) from the source $s$ to the sink $t$ through the node $*$, and now sends packages through it. In figure \ref{fig:changed_route2} node $*$ has moved away, and the route $r$ can no longer be used, and the routing algorithm has to find a new route.}
