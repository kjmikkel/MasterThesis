...
GREEDYNeighbors::GREEDYNeighbors(){
  my_id_ = -1;
  my_x_ = 0.0;
  my_y_ = 0.0;
...
GREEDYNeighbors::~GREEDYNeighbors(){
  struct greedy_neighbor *temp = head_;
  while(temp){
    temp = temp->next_;
    free(head_);
    head_ = temp;
...
double 
GREEDYNeighbors::getdis(double ax, double ay, double bx, double by){
  double tempx = ax - bx;
  double tempy = ay - by;
...
int
GREEDYNeighbors::nbsize(){
  return nbSize_;
}

void
GREEDYNeighbors::myinfo(nsaddr_t mid, double mx, double my){
  my_id_ = mid;
  my_x_ = mx;
  my_y_ = my;
}
...
GREEDYNeighbors::getnb(nsaddr_t nid){
  struct greedy_neighbor *temp = head_;
  while(temp){
    if(temp->id_ == nid){
      if((GREEDY_CURRENT - temp->ts_) < DEFAULT_GREEDY_TIMEOUT)
	return temp;
      else {
...
void 
GREEDYNeighbors::newNB(nsaddr_t nid, double nx, double ny){
  struct greedy_neighbor *temp = getnb(nid);

  if(temp==NULL){ //it is a new neighbor
    temp=(struct greedy_neighbor*)malloc(sizeof(struct greedy_neighbor));
    temp->id_ = nid;
    temp->x_ = nx;
    temp->y_ = ny;
    temp->ts_ = GREEDY_CURRENT;
...
void
GREEDYNeighbors::delnb(nsaddr_t nid){
  struct greedy_neighbor *temp = getnb(nid);
  if(temp==NULL) return;
  else delnb(temp);
}
...
void
GREEDYNeighbors::delnb(struct greedy_neighbor *nb){
  struct greedy_neighbor *preffix = nb->prev_;

  // If we are working with the first element
  if(preffix == NULL){
    head_ = nb->next_;
    nb->next_ = NULL;

    // If we are working with the last element
    if(head_ == NULL) 
      tail_ = NULL;
    ...
    nb->prev_ = NULL;
    // If the prefix is now the last element
...
void
GREEDYNeighbors::delalltimeout(){
  struct greedy_neighbor *temp = head_;
  struct greedy_neighbor *dd;
  while(temp){
    if((GREEDY_CURRENT - temp->ts_) >= DEFAULT_GREEDY_TIMEOUT){
      dd = temp;
      temp = temp->next_;
...
nsaddr_t
GREEDYNeighbors::gf_nexthop(double dx, double dy){
  struct greedy_neighbor *temp = head_;
...
struct greedy_neighbor *
GREEDYNeighbors::gg_planarize(){
    struct greedy_neighbor *temp, *result, *index;
    index = head_;
    result = NULL;
...
double
GREEDYNeighbors::angle(double x1, double y1, double x2, double y2){
 ...
 */
int
GREEDYNeighbors::intersect(nsaddr_t theother, double sx, double sy,
			 double dx, double dy){
  //line 1 (x1,y1)--(x2,y2) is the segment
  //line 2 (x3,y3)--(x4,y4) is the xD 
  struct greedy_neighbor *other = getnb(theother);
...
int
GREEDYNeighbors::num_of_neighbors(struct greedy_neighbor *nblist){
  struct greedy_neighbor *temp = nblist;
  int counter = 0;
  while(temp){
    counter++;
    temp = temp->next_;
...
void
GREEDYNeighbors::free_neighbors(struct greedy_neighbor *nblist){
  struct greedy_neighbor *temp, *head;
  head = nblist;
  while(head){
    temp = head;
    head = head->next_;
...
nsaddr_t
GREEDYNeighbors::peri_nexthop(int type_, nsaddr_t last,
			    double sx, double sy,
			    double dx, double dy){
  struct greedy_neighbor *planar_neighbors, *temp;
  ...
  if(last>-1){
    struct greedy_neighbor *lastnb = getnb(last);
    if(lastnb == NULL) {
