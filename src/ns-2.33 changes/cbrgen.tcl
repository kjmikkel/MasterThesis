...
proc usage {} {
    ...
    puts "\nusage: $argv0 \[-type cbr|tcp\] \[-nn nodes\] \[-seed seed\] \[-mc connections\] \[-rate rate\] \[-max time\]\n"
}

proc getopt {argc argv} {
	global opt
	lappend optlist nn seed mc rate type max
...
proc create-cbr-connection { src dst } {
	global rng cbr_cnt opt
        ...
        set stime [$rng uniform 0.0 [expr [expr $opt(max) - 60] * 1.0]
...
proc create-tcp-connection { src dst } {
    ...
    set stime [$rng uniform 0.0 [expr $opt(max) * 1.0]]
