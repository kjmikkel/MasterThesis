# GREEDY.tcl -- TCL-world configuration of GREEDY routing for ns-2
...
# ======================================================================
Agent/GREEDY set sport_        0
Agent/GREEDY set dport_        0
Agent/GREEDY set bint_         0.5 ;# beacon interval
...
Agent/GREEDY set use_mac_      0        ;# use link breakage feedback from MAC
Agent/GREEDY set use_peri_     0	      ;# probe and use perimeters
Agent/GREEDY set verbose_      0        ;# 
Agent/GREEDY set drop_debug_   0        ;#
Agent/GREEDY set peri_proact_  1	      ;# proactively generate peri probes
Agent/GREEDY set use_implicit_beacon_ 0 ;# all packets act as beacons; promisc.
Agent/GREEDY set use_planar_   0        ;# planarize graph
Agent/GREEDY set use_loop_detect_ 0     ;# look for unexpected loops in peris
Agent/GREEDY set use_timed_plnrz_ 0     ;# replanarize periodically

# ->
set opt(ragent)         Agent/GREEDY
# <- inserted - mk
set opt(pos)		NONE			;# Box or NONE

if { $opt(pos) == "Box" } {
	puts "*** GREEDY using Box configuration..."
}
...
Agent/GREEDY instproc init args {
    eval $self next $args
}       
...
proc create-greedy-routing-agent { node id } {
    ...
    $ns_ at 0.0 "$ragent_($id) start-greedy"	;# start updates
...
proc greedy-create-mobile-node { id args } {
